...
public class MergeWizard extends Wizard {
  ...
  public static final String LAST_RELATIVE_PATH_CHOICE = "MergeWizard.relativePath"; // $NON-NLS-1$
  public static final String LAST_RETRIEVE_ELIGIBLE_REVISIONS_SEPARATELY =
      "MergeWizard.retrieveEligibleRevisionsSeparately"; //$NON-NLS-1$
  ...
  public void addPages() {
    ...
    if (changeSetMergeInputs == null) {
      mainPage =
          new MergeWizardMainPage(
              "main",
              Messages.MergeWizard_selectType,
              Activator.getDefault().getImageDescriptor(Activator.IMAGE_MERGE_WIZARD),
              mergeInputProviders); //$NON-NLS-1$
      addPage(mainPage);
    }
    //		else {
    //			resourceSelectionPage = new MergeWizardResourceSelectionPage("resourceSelection", "Select
    // the merge target", Activator.getDefault().getImageDescriptor(Activator.IMAGE_MERGE_WIZARD));
    //			addPage(resourceSelectionPage);
    //		}

    bestPracticesPage =
        new MergeWizardBestPracticesPage(
            "bestPractices",
            Messages.MergeWizard_bestPractices,
            Activator.getDefault().getImageDescriptor(Activator.IMAGE_MERGE_WIZARD)); // $NON-NLS-1$
    ...
    for (int i = 0; i < mergeInputProviders.length; i++) {
      ...
      if (changeSetMergeInputs != null
          && mergeInputProviders[i] instanceof IChangeSetMergeInputProvider)
      ...
      if (providerPages != null) {
        if (changeSetMergeInputs == null
          ...
          for (int j = 0; j < providerPages.length; j++) {
            ...
            if (changeSetMergeInputs != null
                && providerPages[j] instanceof IMergeWizardChangeSetPage)
    ...
    lastPage =
        new MergeWizardLastPage(
            "last",
            Messages.MergeWizard_selectOptions,
            Activator.getDefault().getImageDescriptor(Activator.IMAGE_MERGE_WIZARD)); // $NON-NLS-1$
    addPage(lastPage);
  }

  public boolean canFinish() {
    if ((mainPage != null && !mainPage.isPageComplete()) || !lastPage.isPageComplete())
      return false;
    if ((mainPage != null && mainPage.showBestPracticesPage())
        && !bestPracticesPage.isPageComplete()) return false;
  ...
  public IWizardPage getNextPage(IWizardPage page) {
    ...
    if (page instanceof MergeWizardBestPracticesPage) {
      ...
      if (changeSetMergeInputProvider == null) return providerPages[0];
      else return providerPages[1];
    }
    IWizardPage nextPage;
    if (changeSetMergeInputProvider == null)
      nextPage = mainPage.getSelectedMergeInputProvider().getNextPage(page);
    else nextPage = changeSetMergeInputProvider.getNextPage(page);
    if (nextPage == null) {
      if (mainPage != null && mainPage.getSelectedMergeInputProvider().showOptionsPage())
        return lastPage;
      if (changeSetMergeInputProvider != null && changeSetMergeInputProvider.showOptionsPage())
        return lastPage;
    }
    return nextPage;
  }
  ...
  public boolean performFinish() {
    ...
    else selectedMergeInputProvider = changeSetMergeInputProvider;
  ...
  public boolean suggestMergeSources() {
    return SVNUIPlugin.getPlugin()
        .getPreferenceStore()
        .getBoolean(ISVNUIConstants.PREF_SUGGEST_MERGE_SOURCES);
  }
  ...
  public IMergeInputProvider getSelectedMergeInputProvider() {
    if (changeSetMergeInputProvider == null) return mainPage.getSelectedMergeInputProvider();
    else return changeSetMergeInputProvider;
  }
