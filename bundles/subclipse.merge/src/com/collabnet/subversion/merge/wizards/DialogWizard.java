...
public class DialogWizard extends Wizard {
  ...
  public void addPages() {
    ...
    if (type == FINISHED_EDITING) {
      ...
      boolean propertyConflict =
          conflictDescriptor != null
              && conflictDescriptor.getConflictKind() == SVNConflictDescriptor.Kind.property;
      finishedEditingWizardPage =
          new FinishedEditingWizardPage("finishedEditing", propertyConflict); // $NON-NLS-1$
    ...
    if (type == CONFLICTS_RESOLVED) {
      ...
      ConflictsResolvedWizardPage conflictsResolvedWizardPage =
          new ConflictsResolvedWizardPage("conflictsResolved"); // $NON-NLS-1$
    ...
    if (type == UNDO_MERGE_WARNING) {
      ...
      QuestionWizardPage questionWizardPage =
          new QuestionWizardPage(
              "question",
              Messages.DialogWizard_undoTitle,
              Messages.DialogWizard_revertWarning); // $NON-NLS-1$
    ...
    if (type == UNDO_MERGE_COMPLETED) {
      ...
      QuestionWizardPage questionWizardPage =
          new QuestionWizardPage(
              "question",
              Messages.DialogWizard_undoTitle,
              Messages.DialogWizard_confirmRevert); // $NON-NLS-1$
    ...
    if (type == UNRESOLVED_CONFLICTS) {
      ...
      UnresolvedConflictsWizardPage unresolvedPage =
          new UnresolvedConflictsWizardPage("unresolved"); // $NON-NLS-1$
    ...
    if (type == PROPERTY_VALUE_SELECTION) {
      ...
      propertyValueSelectionWizardPage =
          new PropertyValueSelectionWizardPage("propertyValueSelection"); // $NON-NLS-1$
    ...
    if (type == COMMIT_MERGEINFO_PROPERTY) {
      ...
      QuestionWizardPage questionWizardPage =
          new QuestionWizardPage(
              "question", Messages.DialogWizard_commitMergeInfoTitle, message); // $NON-NLS-1$
  ...
  public boolean performFinish() {
    ...
    if (type == CONFLICTS_RESOLVED) {
      if (mergeOutputs[0].isIncomplete()) mergeOutputs[0].resume();
      else mergeOutputs[0].delete();
    }
    if (type == PROPERTY_VALUE_SELECTION) {
      conflictResolved = true;
      valueToUse = propertyValueSelectionWizardPage.getValue();
    }
    if (type == MARK_RESOLVED) resolution = resolveConflictWizardPage.getConflictResolution();
    return true;
  }
