...
public class ConflictHandlingWizardPage extends WizardPage {
  ...
  private static final String LAST_TEXT_CHOICE =
  ...
  public ConflictHandlingWizardPage(String pageName) {
    super(
        pageName,
        Messages.ConflictHandlingWizardPage_handleConflict,
  ...
  public void createControl(Composite parent) {
    ...
    IResource conflictResource =
        SVNWorkspaceRoot.getResourceFor(this.resource, new Path(conflictDescriptor.getPath()));
    ...
    if (conflictResource == null) {
      ...
      else relativePath = conflictDescriptor.getPath();
      label.setText(
          Messages.ConflictHandlingWizardPage_resource + convertTempFileName(relativePath));
    } else
      label.setText(
          Messages.ConflictHandlingWizardPage_resource
              + conflictResource.getFullPath().makeRelative().toOSString());
    ...
    conflictGroup.setText(Messages.ConflictHandlingWizardPage_question);
    ...
    markConflictedButton.setText(Messages.ConflictHandlingWizardPage_markConflicted);
    if (conflictDescriptor.isTreeConflict()) {
      chooseIncomingVersionButton = new Button(conflictGroup, SWT.RADIO);
      chooseIncomingVersionButton.setText(Messages.ConflictHandlingWizardPage_0);
    } else {
      if (conflictDescriptor.getMyPath() != null) {
        ...
        if (conflictDescriptor.getConflictKind() == SVNConflictDescriptor.Kind.property) {
          chooseUserVersionButton.setText(Messages.ConflictHandlingWizardPage_useMineProperty);
        } else {
          chooseUserVersionButton.setText(Messages.ConflictHandlingWizardPage_useMine);
          if (!conflictDescriptor.isBinary()) {
            chooseUserVersionForConflictsButton = new Button(conflictGroup, SWT.RADIO);
            chooseUserVersionForConflictsButton.setText(Messages.ConflictHandlingWizardPage_1);
          }
        }
      }
      if (conflictDescriptor.getTheirPath() != null) {
        chooseIncomingVersionButton = new Button(conflictGroup, SWT.RADIO);
        if (conflictDescriptor.getConflictKind() == SVNConflictDescriptor.Kind.property) {
          chooseIncomingVersionButton.setText(
              Messages.ConflictHandlingWizardPage_useIncomingProperty);
        } else {
          chooseIncomingVersionButton.setText(Messages.ConflictHandlingWizardPage_useIncoming);
          if (!conflictDescriptor.isBinary()) {
            chooseIncomingVersionForConflictsButton = new Button(conflictGroup, SWT.RADIO);
            chooseIncomingVersionForConflictsButton.setText(Messages.ConflictHandlingWizardPage_2);
          }
        }
      }
      //
      // Do not expose this option until merge bug is resolved.
      //
      //		if (conflictDescriptor.getBasePath() != null) {
      //			chooseBaseVersionButton = new Button(conflictGroup, SWT.RADIO);
      //			if (conflictDescriptor.getConflictKind() == SVNConflictDescriptor.Kind.property)
      //				chooseBaseVersionButton.setText("Resolve the conflict by using the &base version of the
      // property.");
      //			else
      //				chooseBaseVersionButton.setText("Resolve the conflict by using the &base version of the
      // file.");
      ...
      if (!conflictDescriptor.isBinary()) {
        if (conflictDescriptor.getConflictKind() != SVNConflictDescriptor.Kind.property
            && fileExists(conflictDescriptor.getMergedPath())) {
          fileEditorButton = new Button(conflictGroup, SWT.RADIO);
          fileEditorButton.setText(Messages.ConflictHandlingWizardPage_editWithMarkers);
        }
      }
      if (showConflictEditorOption()) {
        conflictEditorButton = new Button(conflictGroup, SWT.RADIO);
        if (conflictDescriptor.getConflictKind() == SVNConflictDescriptor.Kind.property
            && conflictDescriptor.getMergedPath() == null)
          conflictEditorButton.setText(Messages.ConflictHandlingWizardPage_reviewProperties);
        else conflictEditorButton.setText(Messages.ConflictHandlingWizardPage_graphicalEditor);
      }
    }
    ...
    if (lastChoice == ConflictResolution.CONFLICT_EDITOR && conflictEditorButton != null)
      conflictEditorButton.setSelection(true);
    if (!conflictDescriptor.isBinary()) {
      if (conflictDescriptor.getConflictKind() != SVNConflictDescriptor.Kind.property
          && lastChoice == ConflictResolution.FILE_EDITOR) fileEditorButton.setSelection(true);
    }
    ...
    SelectionListener selectionListener =
        new SelectionAdapter() {
          public void widgetSelected(SelectionEvent e) {
            if (conflictDescriptor.isBinary()) {
              if (markConflictedButton.getSelection())
                settings.put(LAST_BINARY_CHOICE, ISVNConflictResolver.Choice.postpone);
              else if (chooseUserVersionButton != null && chooseUserVersionButton.getSelection())
                settings.put(LAST_BINARY_CHOICE, ISVNConflictResolver.Choice.chooseMineFull);
              else if (chooseIncomingVersionButton != null
                  && chooseIncomingVersionButton.getSelection())
                settings.put(LAST_BINARY_CHOICE, ISVNConflictResolver.Choice.chooseTheirsFull);
              else if (chooseBaseVersionButton != null && chooseBaseVersionButton.getSelection())
                settings.put(LAST_BINARY_CHOICE, ISVNConflictResolver.Choice.chooseBase);
            } else {
              if (markConflictedButton.getSelection())
                settings.put(LAST_TEXT_CHOICE, ISVNConflictResolver.Choice.postpone);
              else if (chooseUserVersionButton != null && chooseUserVersionButton.getSelection())
                settings.put(LAST_TEXT_CHOICE, ISVNConflictResolver.Choice.chooseMineFull);
              else if (chooseUserVersionForConflictsButton != null
                  && chooseUserVersionForConflictsButton.getSelection())
                settings.put(LAST_TEXT_CHOICE, ISVNConflictResolver.Choice.chooseMine);
              else if (chooseIncomingVersionButton != null
                  && chooseIncomingVersionButton.getSelection())
                settings.put(LAST_TEXT_CHOICE, ISVNConflictResolver.Choice.chooseTheirsFull);
              else if (chooseIncomingVersionForConflictsButton != null
                  && chooseIncomingVersionForConflictsButton.getSelection())
                settings.put(LAST_TEXT_CHOICE, ISVNConflictResolver.Choice.chooseTheirs);
              else if (chooseBaseVersionButton != null && chooseBaseVersionButton.getSelection())
                settings.put(LAST_TEXT_CHOICE, ISVNConflictResolver.Choice.chooseBase);
              else if (fileEditorButton != null && fileEditorButton.getSelection())
                settings.put(LAST_TEXT_CHOICE, ConflictResolution.FILE_EDITOR);
            }
            if (conflictEditorButton != null && conflictEditorButton.getSelection())
              settings.put(LAST_TEXT_CHOICE, ConflictResolution.CONFLICT_EDITOR);
          }
        };

    markConflictedButton.addSelectionListener(selectionListener);
    if (chooseIncomingVersionButton != null)
    ...
    if (chooseUserVersionButton != null)
    ...
    if (chooseBaseVersionButton != null)
    ...
    if (conflictDescriptor.isTreeConflict())
      applyToAllButton.setText(Messages.ConflictHandlingWizardPage_3);
    else if (conflictDescriptor.getConflictKind() == SVNConflictDescriptor.Kind.property)
      applyToAllButton.setText(Messages.ConflictHandlingWizardPage_applyToAllProperties);
    else if (conflictDescriptor.isBinary())
      applyToAllButton.setText(Messages.ConflictHandlingWizardPage_applyToAllBinary);
    else applyToAllButton.setText(Messages.ConflictHandlingWizardPage_applyToAllText);

    File file = new File(conflictDescriptor.getPath());
    if (conflictDescriptor.isTreeConflict())
      setMessage(
          "Tree conflict on "
              + convertTempFileName(file.getName())
              + "."); //$NON-NLS-1$ //$NON-NLS-1$ //$NON-NLS-2$
    else if (conflictDescriptor.getConflictKind() == SVNConflictDescriptor.Kind.property)
      setMessage(
          Messages.ConflictHandlingWizardPage_cannotMergeProperty
              + conflictDescriptor.getPropertyName()
              + Messages.ConflictHandlingWizardPage_forResource
              + convertTempFileName(file.getName())
              + "."); //$NON-NLS-1$
    else
      setMessage(
          Messages.ConflictHandlingWizardPage_cannotMerge
              + convertTempFileName(file.getName())
              + "."); //$NON-NLS-1$ //$NON-NLS-1$
  ...
  public ConflictResolution getConflictResolution() {
    ...
    if (!conflictDescriptor.isBinary()) {
      if (fileEditorButton != null && fileEditorButton.getSelection())
        resolution = ConflictResolution.FILE_EDITOR;
    }
    if (conflictEditorButton != null && conflictEditorButton.getSelection())
      resolution = ConflictResolution.CONFLICT_EDITOR;
  ...
  private boolean showConflictEditorOption() {
    ...
    if (conflictDescriptor.isBinary()) {
      ...
      try {
        MergeFileAssociation[] mergeFileAssociations =
            SVNUIPlugin.getPlugin().getMergeFileAssociations();
        if (mergeFileAssociations != null) {
          for (int i = 0; i < mergeFileAssociations.length; i++) {
            if (mergeFileAssociations[i].matches(pathFile.getName())
                || mergeFileAssociations[i].getFileType().equals(pathFile.getName())) {
              return true;
            }
          }
        }
