    ...
    if (showDialog) {
      ...
      if (resources.size() > 1) {
        if (!MessageDialog.openConfirm(
            getShell(),
            Messages.MergeViewResolveAction_confirm,
            Messages.MergeViewResolveAction_confirmMultiple)) return;
        ...
        if (treeConflict == null) {
          String message =
              Messages.MergeViewResolveAction_confirmTreeConflict
                  + resource.getName()
                  + "?"; //$NON-NLS-1$
          if (!MessageDialog.openConfirm(
              getShell(), Messages.MergeViewResolveAction_confirm, message)) return;
          setResolution(ISVNConflictResolver.Choice.chooseMerged);
        } else {
          ResolveTreeConflictWizard wizard =
              new ResolveTreeConflictWizard(treeConflict, getTargetPart());
          WizardDialog dialog =
              new SizePersistedWizardDialog(
                  Display.getDefault().getActiveShell(),
                  wizard,
                  "ResolveTreeConflict"); //$NON-NLS-1$
        ...
        IResource resource = (IResource) resources.get(0);
        String message =
            Messages.MergeViewResolveAction_confirmProperty
        ...
        MergeWizardDialog resolveDialog =
            new MergeWizardDialog(Display.getDefault().getActiveShell(), dialogResolveWizard);
                ...
                try {
                  LocalResourceStatus status =
                      SVNProviderPlugin.getPlugin()
                          ...
                          .getStatus(checkResult.getResource());
      ...
      while (iter.hasNext()) {
        ...
        if (selectedObject instanceof MergeResult) {
          ...
          if (mergeResult.isResolved()
              && mergeResult.isPropertyResolved()
