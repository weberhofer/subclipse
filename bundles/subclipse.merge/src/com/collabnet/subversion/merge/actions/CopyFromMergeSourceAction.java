    ...
    while (iter.hasNext()) {
      ...
      if (object instanceof ISkippedMergeResult) {
          ...
          if (!resource.exists()) {
            String resourceSubString =
                resource
                    .getFullPath()
                    .toOSString()
                    .substring(skippedMergeResult.getMergeOutput().getTarget().length() + 1);
            ...
            try {
              ...
              MessageDialog.openError(
                  getShell(), Messages.CopyFromMergeSourceAction_title, e.getLocalizedMessage());
              return;
            }
          }
        }
          ...
          if (!resource.exists()) {
            String resourceSubString =
                resource
                    .getFullPath()
                    .toOSString()
                    .substring(skippedMergeResultsFolder.getMergeOutput().getTarget().length() + 1);
            if (!resourceSubString.startsWith("/"))
              resourceSubString = "/" + resourceSubString; // $NON-NLS-1$ //$NON-NLS-2$
            try {
              srcUrl =
        ...
        if (resource.exists()) {
          MessageDialog.openError(
              getShell(),
              Messages.CopyFromMergeSourceAction_title,
              resource.getName() + Messages.CopyFromMergeSourceAction_alreadyExists);
          return;
        }
        ...
        CopyOperation copyOperation =
            new CopyOperation(
                getTargetPart(), resources, srcUrl, destPath, svnRevision, mergeResult);
        copyOperation.run();
      }
    }
  }
