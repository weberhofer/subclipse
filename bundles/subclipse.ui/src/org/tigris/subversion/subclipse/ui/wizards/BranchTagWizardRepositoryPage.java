...
public class BranchTagWizardRepositoryPage extends SVNWizardPage {
  ...
  public void createControl(Composite parent) {
    ...
    toUrlCombo.init(
        resources == null
            ? "repositoryBrowser"
            : resources[0].getProject().getName()); // $NON-NLS-1$
    ...
    makeParentsButton.setSelection(
        settings.getBoolean("BranchTagDialog.makeParents")); // $NON-NLS-1$
    makeParentsButton.addSelectionListener(
        new SelectionAdapter() {
          public void widgetSelected(SelectionEvent e) {
            settings.put(
                "BranchTagDialog.makeParents", makeParentsButton.getSelection()); // $NON-NLS-1$
          }
        });
    ...
    if (multipleSelections() && !sameParents()) {
      ...
      sameStructureButton.setSelection(
          settings.getBoolean("BranchTagDialog.sameStructure")); // $NON-NLS-1$
      sameStructureButton.addSelectionListener(
          new SelectionAdapter() {
            public void widgetSelected(SelectionEvent e) {
              settings.put(
                  "BranchTagDialog.sameStructure",
                  sameStructureButton.getSelection()); // $NON-NLS-1$
              viewer.refresh();
            }
          });
    }
  ...
  private boolean multipleSelections() {
    return (resources != null && resources.length > 1)
        || (remoteResources != null && remoteResources.length > 1);
  }

  private boolean sameParents() {
    for (int i = 0; i < branchResources.length; i++) {
      String name = null;
      if (branchResources[i].getResource() == null)
        name = branchResources[i].getRemoteResource().getName();
  ...
  public String getCommonRoot() {
    ...
    if (resources == null) {
      ...
      for (int i = 0; i < remoteResources.length; i++) {
        branchResources[i] =
            new BranchResource(
                remoteResources[i], urlStrings[i].substring(commonRoot.length() + 1));
      }
    } else {
      branchResources = new BranchResource[resources.length];
      for (int i = 0; i < resources.length; i++) {
        branchResources[i] =
            new BranchResource(resources[i], urlStrings[i].substring(commonRoot.length() + 1));
      }
    }
  ...
  class BranchLabelProvider extends LabelProvider implements ITableLabelProvider {
    ...
    public String getText(Object element) {
      ...
      if (multipleSelections()) {
        if (sameStructureButton != null && sameStructureButton.getSelection())
          return branchResource.getPartialPath()
              + " ["
              + toUrlCombo.getText()
              + "/"
              + branchResource.getPartialPath()
              + "]";
        else {
    ...
    public Image getImage(Object element) {
      ...
      if (branchResource.getResource() == null)
        return workbenchLabelProvider.getImage(branchResource.getRemoteResource());
