...
/**
 * Implements the ISVNLocalFolder interface on top of an instance of the ISVNFolder interface
 *
 * @see ISVNLocalFolder
 */
public class LocalFolder extends LocalResource implements ISVNLocalFolder {
  ...
  /**
   * create a handle based on the given local resource. Container can be IResource.ROOT
   *
   * @param container
   */
  ...
  public ISVNResource[] members(IProgressMonitor monitor, int flags) throws SVNException {
    ...
    final boolean includeManaged =
        (((flags & MANAGED_MEMBERS) != 0)
            || ((flags & (MANAGED_MEMBERS | UNMANAGED_MEMBERS | IGNORED_MEMBERS)) == 0));
    final boolean includeUnmanaged =
        (((flags & UNMANAGED_MEMBERS) != 0)
            || ((flags & (MANAGED_MEMBERS | UNMANAGED_MEMBERS | IGNORED_MEMBERS)) == 0));
    final boolean includeIgnored = ((flags & IGNORED_MEMBERS) != 0);
    final boolean includeExisting =
        (((flags & EXISTING_MEMBERS) != 0)
            || ((flags & (EXISTING_MEMBERS | PHANTOM_MEMBERS)) == 0));
    final boolean includePhantoms =
        (((flags & PHANTOM_MEMBERS) != 0) || ((flags & (EXISTING_MEMBERS | PHANTOM_MEMBERS)) == 0));
  ...
  public void refreshStatus(boolean recursive) throws SVNException {
    SVNProviderPlugin.getPlugin()
        .getStatusCacheManager()
        .refreshStatus((IContainer) resource, recursive);
  }
  ...
  public boolean isDirty() throws SVNException {
    ...
    ISVNLocalResource[] children =
        (ISVNLocalResource[]) members(new NullProgressMonitor(), ALL_UNIGNORED_MEMBERS);
  ...
  public IFolder[] getSVNFolders(IProgressMonitor monitor, final boolean unmanage)
      throws SVNException {
    ...
    SVNProviderPlugin.run(
        new ISVNRunnable() {
          public void run(IProgressMonitor pm) throws SVNException {
            ...
            ISVNResource[] members =
                members(Policy.subMonitorFor(pm, 20), FOLDER_MEMBERS | MANAGED_MEMBERS);
          ...
          private void recursiveGetSVNFolders(
              IContainer container, IProgressMonitor pm, boolean unmanage) {
            try {
              // We must not add svn directories for linked resources.
              if (container.isLinked()) return;
  ...
  public LocalResourceStatus getStatus() throws SVNException {
    if (getIResource().isTeamPrivateMember()
        && (SVNProviderPlugin.getPlugin().isAdminDirectory(getIResource().getName()))) {
