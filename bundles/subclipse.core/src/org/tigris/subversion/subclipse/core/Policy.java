...
public class Policy {
  ...
  /**
   * The initial guess for number of resources(pm ticks) the infinite subPm should start with in
   * case of checkout
   */
  public static final int INFINITE_PM_GUESS_FOR_CHECKOUT = 1000;
  /**
   * The initial guess for number of resources(pm ticks) the infinite subPm should start with in
   * case of update/switch
   */
  public static final int INFINITE_PM_GUESS_FOR_SWITCH = 100;

  private static final String BUNDLE_NAME =
      "org.tigris.subversion.subclipse.core.messages"; //$NON-NLS-1$
  ...
  static {
    ...
    if (SVNProviderPlugin.getPlugin().isDebugging()) {
      DEBUG_METAFILE_CHANGES =
          "true"
              .equalsIgnoreCase(
                  Platform.getDebugOption(
                      SVNProviderPlugin.ID + "/metafiles")); // $NON-NLS-1$ //$NON-NLS-2$
      DEBUG_THREADING =
          "true"
              .equalsIgnoreCase(
                  Platform.getDebugOption(
                      SVNProviderPlugin.ID + "/threading")); // $NON-NLS-1$ //$NON-NLS-2$
      DEBUG_STATUS =
          "true"
              .equalsIgnoreCase(
                  Platform.getDebugOption(
                      SVNProviderPlugin.ID + "/status")); // $NON-NLS-1$ //$NON-NLS-2$
    }
  }
   ...
   * @param style one of
   *     <ul>
   *       <li><code>SubProgressMonitor#SUPPRESS_SUBTASK_LABEL</code>
   *       <li><code>SubProgressMonitor#PREPEND_MAIN_LABEL_TO_SUBTASK</code>
   *     </ul>
   *
  ...
  /**
   * Return a submonitor for cases when we do not know the number of ticks ... The main task label
   * will be prepended to the subtask label.
   *
   * @param monitor the parent progress monitor
   * @param ticks the number of work ticks allocated from the parent monitor
   * @return IProgressMonitor
